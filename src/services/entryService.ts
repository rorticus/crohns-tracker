...
export class EntryService {
  ...
  static async createBowelMovement(input: CreateBowelMovementInput): Promise<BowelMovementEntry> {
    // Validate input
    const validation = ValidationService.validateBowelMovementInput(input);
    ...
    try {
      // Generate timestamp
      const timestamp = ValidationService.generateTimestamp(input.date, input.time);
  ...
  // Create note entry
  static async createNote(input: CreateNoteInput): Promise<NoteEntry> {
    // Validate input
    const validation = ValidationService.validateNoteInput(input);
    if (!validation.isValid) {
      throw new EntryServiceError(
        `Validation failed: ${validation.errors.map(e => e.message).join(', ')}`,
        'VALIDATION_ERROR'
  ...
  static async getEntriesForDate(date: string): Promise<CombinedEntry[]> {
    try {
  ...
  static async updateBowelMovement(
    ...
    try {
      ...
      if (!existingEntry || existingEntry.type !== 'bowel_movement') {
        throw new EntryServiceError('Bowel movement entry not found', 'NOT_FOUND');
      }
      ...
      if (updates.consistency !== undefined) bowelMovementUpdates.consistency = updates.consistency;
      if (updates.urgency !== undefined) bowelMovementUpdates.urgency = updates.urgency;
  ...
  static async getEntry(entryId: number): Promise<CombinedEntry | null> {
    try {
      const result = await db
        .select({
          entry: entries,
  ...
  static async getEntryCount(startDate?: string, endDate?: string): Promise<number> {
    try {
